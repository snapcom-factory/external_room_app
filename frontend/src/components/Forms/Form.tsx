...
export default function Form({ ...props }) {
    ...
    return (
        <form className='wrapper form-container'
        // onSubmit={(post) => handleSubmit(post)}
        >
            <h2 style={{ textAlign: 'center' }}>{props.formTitle}</h2>

            {props.children} {/*  actual inputs */}

            <div style={{ width: '100%' }}>
                <Button fullWidth variant="contained" size="medium" id='primary-btn'
                // type='submit'
                >{props.submitTitle}</Button>
